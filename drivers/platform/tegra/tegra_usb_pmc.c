...
static void usb_phy_power_down_pmc(struct tegra_usb_pmc_data *pmc_data)
{
	...
	tegra_pmc_writel(PMC_POWER_DOWN_MASK, PMC_USB_AO);
	...
	tegra_pmc_writel(val, PMC_SLEEPWALK_REG(0));
	tegra_pmc_writel(val, PMC_SLEEPWALK_REG(1));
	tegra_pmc_writel(val, PMC_SLEEPWALK_REG(2));
	...
	tegra_pmc_writel(val, PMC_SLEEPWALK_UHSIC);

	/* Turn over pad configuration to PMC */
	val = tegra_pmc_readl(PMC_SLEEP_CFG);
	...
	tegra_pmc_writel(val, PMC_SLEEP_CFG);
...
static void utmip_setup_pmc_wake_detect(struct tegra_usb_pmc_data *pmc_data)
{
	...
	*/
	...
	/* config debouncer */
	val = tegra_pmc_readl(PMC_USB_DEBOUNCE);
	val &= ~UTMIP_LINE_DEB_CNT(~0);
	val |= UTMIP_LINE_DEB_CNT(1);
	val |= PMC_USB_DEBOUNCE_VAL(2);
	tegra_pmc_writel(val, PMC_USB_DEBOUNCE);

	/* Make sure nothing is happening on the line with respect to PMC */
	val = tegra_pmc_readl(PMC_UTMIP_FAKE);
	val &= ~USBOP_VAL(inst);
	val &= ~USBON_VAL(inst);
	tegra_pmc_writel(val, PMC_UTMIP_FAKE);

	/* Make sure wake value for line is none */
	val = tegra_pmc_readl(PMC_SLEEPWALK_CFG);
	val &= ~UTMIP_LINEVAL_WALK_EN(inst);
	tegra_pmc_writel(val, PMC_SLEEPWALK_CFG);
	val = tegra_pmc_readl(PMC_SLEEP_CFG);
	...
	* in this case usb_line_wake */
	...
	pmc_pad_cfg_val = tegra_pmc_readl(PMC_PAD_CFG);
	val = tegra_pmc_readl(PMC_TRIGGERS);
	val |= UTMIP_CAP_CFG(inst);
	tegra_pmc_writel(val, PMC_TRIGGERS);
	udelay(1);
	pmc_pad_cfg_val = tegra_pmc_readl(PMC_PAD_CFG);
	...
	if ((pmc_data->port_speed < USB_PMC_PORT_SPEED_UNKNOWN) ||
		...
		* to signal a resume once an wake event is detected */
		val = tegra_pmc_readl(PMC_SLEEPWALK_REG(inst));
		val &= ~UTMIP_AP_A;
		val |= UTMIP_USBOP_RPD_A | UTMIP_USBON_RPD_A | UTMIP_AN_A |
			...
			UTMIP_USBOP_RPD_D | UTMIP_USBON_RPD_D | UTMIP_AP_D |
					UTMIP_AN_D;
		tegra_pmc_writel(val, PMC_SLEEPWALK_REG(inst));
...
static void uhsic_powerup_pmc_wake_detect(struct tegra_usb_pmc_data *pmc_data)
{
	...
	/* Disable PMC master mode by clearing MASTER_EN */
	val = tegra_pmc_readl(PMC_UHSIC_SLEEP_CFG(inst));
	val &= ~(UHSIC_MASTER_ENABLE(inst));
	tegra_pmc_writel(val, PMC_UHSIC_SLEEP_CFG(inst));
...
static void uhsic_powerdown_pmc_wake_detect(struct tegra_usb_pmc_data *pmc_data)
{
	...
	val = UHSIC_STROBE_RPD_A | UHSIC_DATA_RPD_A | UHSIC_STROBE_RPD_B |
		UHSIC_DATA_RPD_B | UHSIC_STROBE_RPD_C | UHSIC_DATA_RPD_C |
		UHSIC_STROBE_RPD_D | UHSIC_DATA_RPD_D;
	tegra_pmc_writel(val, PMC_SLEEPWALK_UHSIC(inst));
...
static void uhsic_setup_pmc_wake_detect(struct tegra_usb_pmc_data *pmc_data)
{
	...
	*/
	...
	/* UTMIP_PWR_PX=1 for power savings mode */
	val = tegra_pmc_readl(PMC_UHSIC_MASTER_CONFIG(inst));
	val |= UHSIC_PWR(inst);
	tegra_pmc_writel(val, PMC_UHSIC_MASTER_CONFIG(inst));
	...
	/* Make sure nothing is happening on the line with respect to PMC */
	val = tegra_pmc_readl(PMC_UHSIC_FAKE(inst));
	val &= ~UHSIC_FAKE_STROBE_VAL(inst);
	val &= ~UHSIC_FAKE_DATA_VAL(inst);
	tegra_pmc_writel(val, PMC_UHSIC_FAKE(inst));

	/* Clear walk enable */
	val = tegra_pmc_readl(PMC_UHSIC_SLEEPWALK_CFG(inst));
	val &= ~UHSIC_LINEVAL_WALK_EN(inst);
	tegra_pmc_writel(val, PMC_UHSIC_SLEEPWALK_CFG(inst));
	...
	* to signal a resume once an wake event is detected */
	...
	val = tegra_pmc_readl(PMC_SLEEPWALK_UHSIC(inst));
	...
	val = tegra_pmc_readl(PMC_UHSIC_TRIGGERS(inst));
	val |= UHSIC_CLR_WAKE_ALARM(inst) | UHSIC_CLR_WALK_PTR(inst);
	tegra_pmc_writel(val, PMC_UHSIC_TRIGGERS(inst));
...
void tegra_usb_pmc_reg_update(u32 reg_offset, u32 mask, u32 val)
{
	...
	reg = tegra_pmc_readl(reg_offset);
	reg &= ~mask;
	reg |= val;
	tegra_pmc_writel(reg, reg_offset);
...
{
	spin_lock_irqsave(&pmc_lock, flags);
	tegra_pmc_writel(val, reg_offset);
	spin_unlock_irqrestore(&pmc_lock, flags);
}
